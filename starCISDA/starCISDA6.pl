...
my $input_g  = '';  ## such as "6-MAPQ10/1_Trim_BWAmem"
my $output_g = '';  ## such as "7-finalBAM/1_Trim_BWAmem"
...
my $HELP = '
        ...
        Step 6: Potential PCR duplicates are removed using "MarkDuplicates" from "Picards".
                Finally, only the UMNRR4 with MAPQ is more than 10 are kept
        ...
        Usage:
               perl  starCISDA6.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]
        For instance:
               perl  starCISDA6.pl   -genome hg38   -in 6-MAPQ10/1_Trim_BWAmem   -out 7-finalBAM/1_Trim_BWAmem    > starCISDA6.runLog
...
## Version Infromation
my $version = "    The Sixth Step of starCISDA (small-scale TELP-assisted rapid ChIP–seq Data Analyzer), version 0.9.0, 2017-10-01.";
...
$input_g  = '6-MAPQ10/1_Trim_BWAmem';     ## This is only an initialization value or suggesting value, not default value.
$output_g = '7-finalBAM/1_Trim_BWAmem';   ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  starCISDA6.pl  -help' \n";
    exit 0;
}
...
sub  myQC_BAM_1  {
    ...
    for ( my $i=0; $i<=$#Files; $i++ ) {
        ...
        say    "\t......$temp";
        $temp =~ s/\.t\.bam$//  ||  die;
        system("samtools  sort  -m 2G  -o $dir1/$temp.bam   --output-fmt bam  -T $dir1/yp_$temp   --threads $numCores_g    $dir1/$temp.t.bam    >>$SAMtools/$temp.sort.runLog    2>&1");
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    $temp =~ s/\.bam$//  ||  die; 
    say   "\t......$BAMfiles_g[$i]";
    system("java  -jar   $Picard_g   MarkDuplicates    REMOVE_DUPLICATES=true   INPUT=$input_g/$temp.bam    OUTPUT=$output_g/$temp.t.bam    METRICS_FILE=$output2_g/$temp.marked_dup_metrics.txt   >> $output2_g/$temp.runLog     2>&1 ");
}
###################################################################################################################################################################################################
