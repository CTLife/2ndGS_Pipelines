...
my $output_g = '';  ## such as "6-MAPQ10/1_Trim_BWAmem"
...
my $HELP = '
        ...
        Step 5: Only the mapped reads with MAPQ>10 are retained and reads on chr_random, chr_alt, chrUn and chrM, all of them are removed.
        ...
        Usage:
               perl  starCISDA5.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]
        For instance:
               perl  starCISDA5.pl   -genome hg38   -in 5-rawBAM/1_Trim_BWAmem   -out 6-MAPQ10/1_Trim_BWAmem    > starCISDA5.runLog
...
## Version Infromation
my $version = "    The Fifth Step of starCISDA (small-scale TELP-assisted rapid ChIP–seq Data Analyzer), version 0.9.0, 2017-10-01.";
...
$output_g = '6-MAPQ10/1_Trim_BWAmem';    ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  starCISDA5.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    $temp =~ s/\.bam$//  ||  die; 
    say   "\t......$BAMfiles_g[$i]";
    system(`samtools  view   -h   -F 4    --threads $numCores_g    -q 10     -o $output_g/$temp.t.sam   $input_g/$temp.bam    >> $output2_g/$temp.runLog     2>&1  `);  
