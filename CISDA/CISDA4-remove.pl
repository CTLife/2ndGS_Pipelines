...
my $output_g = '';  ## such as "5-finalBAM-remove/1_BWAmem"
...
my $HELP = '
        ...
        Step 4: Only the mapped reads with MAPQ>20 are retained.
        ...
        Usage:
               perl  CISDA4-remove.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               perl  CISDA4-remove.pl   -in 5-finalBAM/1_BWAmem   -out 5-finalBAM-remove/1_BWAmem    > CISDA4-remove.runLog
...
## Version Infromation
my $version = "    The 4th Step of CISDA (ChIP-Seq Data Analyzer), version 0.9.4,  2018-02-01.";
...
$output_g = '5-finalBAM-remove/1_BWAmem';     ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  CISDA4-remove.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    $temp =~ s/\.sam$//  ||  die; 
    say   "\t......$BAMfiles_g[$i]";
    system(`samtools  view   -h    --threads $numCores_g    -q 20    -o $output_g/$temp.sam   $input_g/$temp.sam    >> $output2_g/$temp.runLog     2>&1  `);  
}
###################################################################################################################################################################################################
