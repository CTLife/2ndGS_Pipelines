...
my $output_g = '';  ## such as "9A-MACS2/1_BWAmem"
...
my $HELP = '
        ...
        Step 8: Call peaks for BAM or BAMPE by using MACS2.  

        Usage:
               perl  CISDA8A.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]
        For instance:
               perl  CISDA8A.pl   -genome hg38   -in 6-BAMPE/1_BWAmem   -out 9A-MACS2/1_BWAmem    > CISDA8A.runLog
...
$output_g = '9A-MACS2/1_BWAmem';  ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  CISDA8A.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#groupFiles_g; $i++ ) { 
    ...
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir1/$temp    --name $temp   --bw 300    --mfold 5 50   --qvalue 0.05                        >> $outputDir1/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir2/$temp    --name $temp   --bw 300    --mfold 5 50   --qvalue 0.05      --broad           >> $outputDir2/$temp.runLog    2>&1`);                    
}
...
for ( my $i=0; $i<=$#groupFiles_g; $i++ ) { 
    ...
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir1/$temp    --name $temp   --bw 300    --mfold 5 50   --qvalue 0.1                 >> $outputDir1/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir2/$temp    --name $temp   --bw 300    --mfold 5 50   --qvalue 0.01                >> $outputDir2/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir3/$temp    --name $temp   --bw 300    --mfold 5 50   --pvalue 0.01                 >> $outputDir3/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir4/$temp    --name $temp   --bw 300    --mfold 5 50   --pvalue 0.001                >> $outputDir4/$temp.runLog    2>&1`);    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir5/$temp    --name $temp   --bw 300    --mfold 5 50   --pvalue 1e-5                 >> $outputDir5/$temp.runLog    2>&1`);   
    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir6/$temp    --name $temp   --bw 300    --mfold 5 50   --qvalue 0.1      --broad                >> $outputDir6/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir7/$temp    --name $temp   --bw 300    --mfold 5 50   --qvalue 0.01     --broad                >> $outputDir7/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir8/$temp    --name $temp   --bw 300    --mfold 5 50   --pvalue 0.01     --broad                >> $outputDir8/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir9/$temp    --name $temp   --bw 300    --mfold 5 50   --pvalue 0.001    --broad                >> $outputDir9/$temp.runLog    2>&1`);    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir10/$temp   --name $temp   --bw 300    --mfold 5 50   --pvalue 1e-5     --broad                >> $outputDir10/$temp.runLog    2>&1`);   
    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir11/$temp   --name $temp   --bw 300    --mfold 5 50   --qvalue 0.05     --broad      --nomodel  --nolambda           >> $outputDir11/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir12/$temp   --name $temp   --bw 300    --mfold 5 50   --pvalue 1e-5     --broad-cutoff 0.3     --broad               >> $outputDir12/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir13/$temp   --name $temp   --bw 300    --mfold 5 50   --pvalue 1e-5     --broad-cutoff 0.01    --broad               >> $outputDir13/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir14/$temp   --name $temp   --bw 300    --mfold 5 50   --pvalue 1e-5     --broad-cutoff 0.001   --broad               >> $outputDir14/$temp.runLog    2>&1`);    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam    --control $input_g/$input_1   --format $format_g   --gsize $genome_size_g   --keep-dup 1     --outdir $outputDir15/$temp   --name $temp   --bw 300    --mfold 5 50   --pvalue 1e-5     --broad-cutoff 0.0001  --broad               >> $outputDir15/$temp.runLog    2>&1`);   
