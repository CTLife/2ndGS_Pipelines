...
my $HELP_g = '
        ...
        Usage:  
               perl  CISDA-3.pl    [-v]    [-h]    [-i inputDir]    [-o outDir]   [-MM N]
        For instance: 
                     perl  CISDA-3.pl    -i 3-Filtered          -o 4-Mapping         -MM 3       
                     perl  CISDA-3.pl    --input 3-Filtered     --output 4-Mapping   --MaxMismatch 3 
                     perl  CISDA-3.pl    --input 3-Filtered     --output 4-Mapping   --MaxMismatch 3   >> CISDA-3.runLog  2>&1
        ...
        -MM N,      --MaxMismatch N           Specify the maximum number of mis-matched bases allowed in the alignment. 
                                              Only for Subread.  default: N is 3.
        ------------------------------------------------------------------------------------------------------------------
...
########## Version Infromation ##########
my $version_g = "  The Third Step of CISDA (ChIP-Seq Data Analyzer), version 0.62, 2016-01-13.";
...
system("subread-align   -v        >> $output2_g/z-version_softwares.txt   2>&1");
...
system("bwa                       >> $output2_g/z-version_softwares.txt   2>&1");
system("echo    '\n\n\n\n\n\n'    >> $output2_g/z-version_softwares.txt   2>&1");
system("echo    '##############################################################################'  >> $output2_g/z-version_softwares.txt   2>&1");

system("bowtie  --version         >> $output2_g/z-version_softwares.txt   2>&1");
system("echo    '\n\n\n\n\n\n'    >> $output2_g/z-version_softwares.txt   2>&1");
system("echo    '##############################################################################'  >> $output2_g/z-version_softwares.txt   2>&1");

system("bowtie2 --version         >> $output2_g/z-version_softwares.txt   2>&1");
system("echo    '\n\n\n\n\n\n'    >> $output2_g/z-version_softwares.txt   2>&1");
system("echo    '##############################################################################'  >> $output2_g/z-version_softwares.txt   2>&1");
...
my $subread2 = "$output_g/1-Subread/Results";
if ( !(-e $subread)  )  { mkdir $subread  || die; }
if ( !(-e $subread2) )  { mkdir $subread2 || die; }
...
for (my $i=0; $i<=$#pairedEnd; $i=$i+2) {  
        ...
        open(tempFH, ">>", "$subread2/z-paired-end-files.txt")  or  die;
        print  tempFH  "$end1,  $end2\n";
        system("subread-align  -T 16  -M $MM_g       --SAMoutput   -i $index_name    -r $input_g/$end1.fastq   -R  $input_g/$end2.fastq   -o  $subread/$temp.sam     -t 1    >>$subread2/$temp.runLog   2>&1");       
}
...
for (my $i=0; $i<=$#singleEnd; $i++) {  
        ...
        my $temp = $1; 
        system("subread-align  -T 16  -M $MM_g      --SAMoutput   -i $index_name    -r $input_g/$temp.fastq                               -o $subread/$temp.sam      -t 1    >>$subread2/$temp.runLog   2>&1");       
}


my $FastQC  = "$subread2/FastQC";
my $QCstat  = "$subread2/QCstat";
...
my $BWAaln2 = "$output_g/2-BWAaln/Results";
if ( !(-e $BWAaln) )   { mkdir $BWAaln  || die; }
if ( !(-e $BWAaln2) )  { mkdir $BWAaln2 || die; }
...
for (my $i=0; $i<=$#pairedEnd; $i=$i+2) {
        ...
        my $end2 = $temp."_2";
        ("$end2.fastq"   eq   $pairedEnd[$i+1])  or  die;
        open(tempFH, ">>", "$BWAaln2/z-paired-end-files.txt")  or  die;
        print  tempFH  "$end1,  $end2\n";
        system("bwa aln       -t 16   -f $BWAaln/$end1.sai      $bwa_index     $input_g/$end1.fastq    >>$BWAaln2/$end1.runLog   2>&1");
        system("bwa aln       -t 16   -f $BWAaln/$end2.sai      $bwa_index     $input_g/$end2.fastq    >>$BWAaln2/$end2.runLog   2>&1");
        system("bwa sampe             -f $BWAaln/$temp.sam      $bwa_index     $BWAaln/$end1.sai  $BWAaln/$end2.sai     $input_g/$end1.fastq  $input_g/$end2.fastq    >>$BWAaln2/$temp.runLog   2>&1"); 
}
...
for (my $i=0; $i<=$#singleEnd; $i++) {   
        ...
        my $temp = $1; 
        system("bwa aln      -t 16   -f $BWAaln/$temp.sai    $bwa_index                          $input_g/$temp.fastq         >>$BWAaln2/$temp.runLog   2>&1");
        system("bwa samse            -f $BWAaln/$temp.sam    $bwa_index     $BWAaln/$temp.sai    $input_g/$temp.fastq         >>$BWAaln2/$temp.runLog   2>&1"); 
}


my $FastQC  = "$BWAaln2/FastQC";
my $QCstat  = "$BWAaln2/QCstat";
if ( !( -e $FastQC)  )   { mkdir $FastQC    ||  die; }
if ( !( -e $QCstat)  )   { mkdir $QCstat    ||  die; }
...
my $BWAmem2 = "$output_g/3-BWA-mem/Results"; 
if ( !(-e $BWAmem) )   { mkdir $BWAmem  || die; }
if ( !(-e $BWAmem2) )  { mkdir $BWAmem2 || die; }
...
for (my $i=0; $i<=$#pairedEnd; $i=$i+2) {
        ...
        my $end2 = $temp."_2";
        ("$end2.fastq"  eq  $pairedEnd[$i+1])  or  die;
        open(tempFH, ">>", "$BWAmem2/z-paired-end-files.txt")  or  die;
        print  tempFH  "$end1,  $end2\n";
...
my $FastQC  = "$BWAmem2/FastQC";
my $QCstat  = "$BWAmem2/QCstat";
if ( !( -e $FastQC)  )   { mkdir $FastQC    ||  die; }
if ( !( -e $QCstat)  )   { mkdir $QCstat    ||  die; }
...
my $Bowtie1_2 = "$output_g/4-Bowtie1/Results"; 
if ( !(-e $Bowtie1)   )  { mkdir $Bowtie1   || die; }
if ( !(-e $Bowtie1_2) )  { mkdir $Bowtie1_2 || die; }
...
for (my $i=0; $i<=$#pairedEnd; $i=$i+2) {  
        ...
        my $end2 = $temp."_2";
        ("$end2.fastq"  eq $pairedEnd[$i+1])  or  die;
        open(tempFH, ">>", "$Bowtie1_2/z-paired-end-files.txt")  or  die;
        print  tempFH  "$end1,  $end2\n";
        system("bowtie   --threads 16   -q  --chunkmbs 640    --sam      --phred33-quals   $Bowtie1_index    -1 $input_g/$end1.fastq   -2 $input_g/$end2.fastq     $Bowtie1/$temp.sam    >>$Bowtie1_2/$temp.runLog   2>&1");       
}
...
for (my $i=0; $i<=$#singleEnd; $i++) {   
        ...
        $singleEnd[$i] =~ m/^(\S+_\S+_\S+_\S+_Rep[1-9])\.fastq$/   or  die; 
        my $temp = $1; 
        system("bowtie   --threads 16   -q  --chunkmbs 640    --sam      --phred33-quals   $Bowtie1_index     $input_g/$temp.fastq                                 $Bowtie1/$temp.sam    >>$Bowtie1_2/$temp.runLog   2>&1");                    
}


my $FastQC  = "$Bowtie1_2/FastQC";
my $QCstat  = "$Bowtie1_2/QCstat";
if ( !( -e $FastQC)  )   { mkdir $FastQC    ||  die; }
if ( !( -e $QCstat)  )   { mkdir $QCstat    ||  die; }
...
my $Bowtie2_2 = "$output_g/5-Bowtie2/Results";
if ( !(-e $Bowtie2)   )  { mkdir $Bowtie2   || die; }
if ( !(-e $Bowtie2_2) )  { mkdir $Bowtie2_2 || die; }
...
for (my $i=0; $i<=$#pairedEnd; $i=$i+2) {  
        ...
        my $end2 = $temp."_2";
        ("$end2.fastq" eq $pairedEnd[$i+1])  or  die;
        open(tempFH, ">>", "$Bowtie2_2/z-paired-end-files.txt")  or  die;
        print  tempFH  "$end1,  $end2\n";
        system("bowtie2   --threads 16   -q   --phred33   --end-to-end    -x $Bowtie2_index    -1 $input_g/$end1.fastq        -2 $input_g/$end1.fastq     -S $Bowtie2/$temp.sam    >>$Bowtie2_2/$temp.runLog  2>&1");                                                                                                                                 
}
...
for (my $i=0; $i<=$#singleEnd; $i++) {   
        ...
        $singleEnd[$i] =~ m/^(\S+_\S+_\S+_\S+_Rep[1-9])\.fastq$/   or  die; 
        my $temp = $1; 
        system("bowtie2   --threads 16   -q   --phred33   --end-to-end    -x $Bowtie2_index    -U $input_g/$temp.fastq                                    -S $Bowtie2/$temp.sam    >>$Bowtie2_2/$temp.runLog  2>&1");                                                                
}


my $FastQC  = "$Bowtie2_2/FastQC";
my $QCstat  = "$Bowtie2_2/QCstat";
if ( !( -e $FastQC)  )   { mkdir $FastQC    ||  die; }
if ( !( -e $QCstat)  )   { mkdir $QCstat    ||  die; }
