...
1. Raw sequencing reads can only be stored in SRA or compressed FASTQ files, because only SRA format and compressed FASTQ format are supported. 
   For compressed FASTQ format, there are only 9 compressed archive file formats are supported: bz2, gz, tar.gz, tar, rar, xz, zip, tar.bz2, tar.xz. 
   Their suffixes must be:  ".bz2",  ".gz",  ".tar.gz",  ".tar",  ".rar",  ".xz",  ".zip", ".tar.bz2", ".tar.xz".
...
2. The names of raw reads files (SRA or compressed FASTQ format) must be: 
   ...
   xxx:       note file format, it can only be sra, fastq.bz2, fastq.gz, fastq.tar.gz, fastq.tar, fastq.rar, fastq.xz,r fastq.zip, fastq.tar.bz2 or fastq.tar.xz. 
...
5. In your work directory, you should create a folder named "0-Other", 
   and put folder "Adapters",  "R_SRC", "Shortcuts", "SRC_Log" and "TXT_Files" and  into the folder "0-Other".                   
