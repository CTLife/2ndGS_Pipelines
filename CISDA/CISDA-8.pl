...
my $HELP_g = '
        ...
        Step 8: Convert BED to bigwig by using bedtools and wigToBigWig.
                Only reads density will be computed, no extend and no shift.
                So single-end and paired-end reads will not be distinguished.
        Usage:  
               perl  CISDA8.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]   
        For instance: 
               perl  CISDA8.pl    -in 8-BED/1_BWA/BED6sortCoord     -out 9-BigWig/1_BWA   >> CISDA8.runLog  2>&1
        ...
        Required arguments:
        -in  inputDir       inputDir is the name of your input folder that contains your BED files, the suffix of the BED files must be ".bed".  (no default)
...
## Version Information
my $version_g = "  The Eighth Step of CISDA (ChIP-Seq Data Analyzer), version 0.7.3, 2016-06-01.";
...
my $input_g   = '8-BED/1_BWA';       ## This is only an initialization  value or suggesting value, not default value.
my $output_g  = '9-BigWig/1_BWA';    ## This is only an initialization  value or suggesting value, not default value.
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {   
        ...
        $temp =~ m/_(Rep[1-9])\.bed$/  or    die   "wrong-2: ## $temp ##";
        if($temp !~ m/^((\d+)_($pattern_g)_(Rep[1-9]))(_[1-2])?\.bed$/) {
             $fileNameBool = 0;
        }
}
...
###################################################################################################################################################################################################
say   "\n\n\n\n\n\n##################################################################################################";
say   "Detecting BED files in input folder ......";
sub numberOfLines  {
    my $filename = $_[0];
    say($filename);
    system("wc -l  $filename  > yp_my_temp_1.xxxxx.txt");
...
my @BEDfiles_g = ();
my @readsNum_g = ();
open(seqFiles_FH, ">", "$output2_g/BED-Files.txt")  or  die; 
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {     
    next unless $inputFiles_g[$i] =~ m/\.bed$/;
    next unless $inputFiles_g[$i] !~ m/^[.]/;
    next unless $inputFiles_g[$i] !~ m/[~]$/;
    ...
    next unless $inputFiles_g[$i] !~ m/^removed_/;
    say    "\t......$inputFiles_g[$i]"; 
    $inputFiles_g[$i] =~ m/^(\d+)_($pattern_g)_(Rep[1-9])\.bed$/  or  die;  
    $BEDfiles_g[$#BEDfiles_g+1] =  $inputFiles_g[$i];
    $readsNum_g[$#readsNum_g+1] =  &numberOfLines("$input_g/$inputFiles_g[$i]");
    say        "\t\t\t\tBED file:  $inputFiles_g[$i]\n";
    say   seqFiles_FH  "BED file:  $inputFiles_g[$i]\n";

}
say   seqFiles_FH  "\n\n\n\n\n";  
say   seqFiles_FH  "All BED files:@BEDfiles_g\n\n\n";
say        "\t\t\t\tAll BED files:@BEDfiles_g\n\n";
my $num1 = $#BEDfiles_g + 1;
say seqFiles_FH   "\nThere are $num1 BED files.\n";
say         "\t\t\t\tThere are $num1 BED files.\n";
###################################################################################################################################################################################################
...
{
say   "\n\n\n\n\n\n##################################################################################################";
say   "Convert BED to BigWig ......";
for (my $i=0; $i<=$#BEDfiles_g; $i++) {
    my $temp = $BEDfiles_g[$i]; 
    $temp =~ s/\.bed$//  ||  die; 
    say   "\t......$BEDfiles_g[$i]";
    my $Temp_scale =  10**7/$readsNum_g[$i];
    print  "\t\t\t\t$BEDfiles_g[$i]:\t$readsNum_g[$i]\t$Temp_scale\n";
    system("\t\tbedtools   genomecov  -bg   -split   -scale $Temp_scale   -i $input_g/$temp.bed   -g 0-Other/Shortcuts/mm10.chrom.sizes    > $output_g/$temp.bedGraph");
    sleep(3);
