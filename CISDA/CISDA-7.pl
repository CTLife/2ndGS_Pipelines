...
my $HELP_g = '
        ...
        Step 7: Convert BAM to BED format by using BEDTools.
        Usage:  
               perl  CISDA7.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]   
        For instance: 
               perl  CISDA7.pl    -in 6-FinalBAM/1_BWA     -out 8-BED/1_BWA   >> CISDA7.runLog  2>&1
        ...
        -out outDir         outDir is the name of your output folder that contains running results (BED format) of this step.  (no default)
        ------------------------------------------------------------------------------------------------------------------
...
## Version Information
my $version_g = "  The Seventh Step of CISDA (ChIP-Seq Data Analyzer), version 0.7.3, 2016-06-01.";
...
my $output_g  = '8-BED/1_BWA';           ## This is only an initialization  value or suggesting value, not default value.
...
if($boole_g == 1) {
    ...
    print   '    Please see help message by using "perl  CISDA7.pl  -help". ';
    print "\n\n";
    exit 0;
}
...
my $BED6_g    = "$output_g/BED6sortCoord";
my $BEDPE_g   = "$output_g/BEDPEsortName";
&myMakeDir($output_g);
&myMakeDir($output2_g);
&myMakeDir($BED6_g);    
&myMakeDir($BEDPE_g);   
...
my $sortName = "$input_g";
$sortName =~ s/^6-FinalBAM/7-SortName/  or  die;
