...
my $output_g = '';  ## such as "14A-ODIN/1_Trim_BWAmem"
...
my $HELP = '
        ...
        Step 13: ODIN is an HMM-based approach to detect and analyse differential peaks in pairs of ChIP-seq data. 
                 ODIN performs genomic signal processing, peak calling and p-value calculation in an integrated framework. 
                 ODIN is tailored for the comparison of two ChIP-seq signals without replicates. If you like to take replicates into account, please use THOR.  

        Usage:
               perl  CISDA13A-ODIN.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]
        For instance:
               perl  CISDA13A-ODIN.pl   -genome hg38   -in 7-finalBAM/1_Trim_BWAmem   -out 14A-ODIN/1_Trim_BWAmem    > CISDA13A-ODIN.runLog
...
## Version Infromation
my $version = "    The 13th Step of CISDA (ChIP-Seq Data Analyzer), version 0.9.0, 2017-10-01.";
...
$output_g = '14A-ODIN/1_Trim_BWAmem';     ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  CISDA13A-ODIN.pl  -help' \n";
    exit 0;
}
...
###################################################################################################################################################################################################
say   "\n\n\n\n\n\n##################################################################################################";
say   "diffPeak calling ......";   

{
my $GenomeFa = "0-Other/Shortcuts/$genome_g/$genome_g.fasta";
my $GenomeSize = "0-Other/Shortcuts/$genome_g/$genome_g.chrom.sizes";
#  -m, --merge           Merge peaks which have a distance less than the estimated fragment size (recommended for histone data). [default: False]
#    --ext-1=200    --ext-2=200     --ext-input-1=200    --ext-input-2=200  



my $chip1  = "$input_g/1_20161206WY-3-3-Ctrl-F1-H3K27me3_Rep1.bam";
my $chip2  = "$input_g/1_20161206WY-3-4-KO-F9-H3K27me3_Rep1.bam";
my $input1 = "$input_g/1_Input-20161206WY-1-1-Ctrl-F1-input_Rep1.bam";
my $input2 = "$input_g/1_Input-20161206WY-1-1-Ctrl-F1-input_Rep1.bam";  
my $temp1 = "H3K27ac.binom";
#my $temp2 = "H3K27ac.poisson-c";
&myMakeDir("$output_g/$temp1");
#&myMakeDir("$output_g/$temp2");
system(" rgt-ODIN  $chip1  $chip2   $GenomeFa   $GenomeSize  --input-1=$input1  --input-2=$input2   --pvalue=0.1   --merge   --name=$temp1    --distr=binom       --verbose    --output-dir=$output_g/$temp1    --foldchange=2.0  --confidence_threshold=0.7   > $output_g/$temp1.runlog  2>&1");                                                           
#system(" rgt-ODIN  $chip1  $chip2   $GenomeFa   $GenomeSize  --input-1=$input1  --input-2=$input2   --pvalue=0.1   --merge   --name=$temp2    --distr=poisson-c   --verbose    --output-dir=$output_g/$temp2    --foldchange=2.0  --confidence_threshold=0.7   > $output_g/$temp2.runlog  2>&1");                                                            
##  filter-ODIN.sh   temp-diffpeaks.bed    2   > filtered.bed
##  split-ODIN.sh  temp-diffpeaks.bed
