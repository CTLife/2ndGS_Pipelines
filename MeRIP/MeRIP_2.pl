...
my $output_g = '';  ## such as "2_removedAdapters"
{
## Help Infromation
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 2: Remove adapters and bases with low quality by using Trimmomatic and/or trim_galore. 
        ...
        Usage:
               perl MeRIP_2.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               nohup time  perl MeRIP_2.pl   -in 1_rawFASTQ   -out 2_removedAdapters    > MeRIP_2.runLog.txt  2>&1    &
...
## Version Infromation
my $version = "    The 2nd Step, version 1.5,  2023-01-30.";
...
$output_g = '2_removedAdapters';    ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl MeRIP_2.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        $temp =~ s/\.fastq\.gz$//   ;
        $temp =~ s/\.fastq$//    ;
        system(  "java  -jar  $Trimmomatic_g  PE   -threads $numCores_g  -summary  $output2_g/Trimmomatic/$temp.statsSummaryFile.txt   $input_g/$end1  $input_g/$end2    $output_g/$end1  $output_g/unpaired-$end1    $output_g/$end2  $output_g/unpaired-$end2      ILLUMINACLIP:$Trimmomatic2_g:2:30:10:3:TRUE   SLIDINGWINDOW:4:15   MINLEN:15    >> $output2_g/Trimmomatic/$temp.Trimmomatic.runLog  2>&1");                                                   
}
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {   
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system("java  -jar  $Trimmomatic_g  SE   -threads $numCores_g  -summary  $output2_g/Trimmomatic/$temp.statsSummaryFile.txt   $input_g/$singleEnd_g[$i]  $output_g/$singleEnd_g[$i]    ILLUMINACLIP:$Trimmomatic2_g:2:30:10:3      SLIDINGWINDOW:4:15  MINLEN:15        >> $output2_g/Trimmomatic/$temp.Trimmomatic.runLog.txt  2>&1");                    
}

my $MultiQC0  = "$output2_g/MultiQC/Trimmomatic";
&myMakeDir($MultiQC0); 
system( "multiqc  --title Trimmomatic    --filename  Trimmomatic   --module  trimmomatic   --verbose  --export  --outdir $MultiQC0   --pdf  --profile-runtime   $output2_g/Trimmomatic   >> $MultiQC0/MultiQC.Trimmomatic.runLog.txt    2>&1" );
...
## keepBothReads: After read-though has been detected by palindrome mode, and the adapter sequence removed, the reverse read contains the same sequence information as the
## forward read, albeit in reverse complement. For this reason, the default behaviour is to entirely drop the reverse read. By specifying „true‟ for this parameter, the reverse read will
## also be retained, which may be useful e.g. if the downstream tools cannot handle a combination of paired and unpaired reads. 
