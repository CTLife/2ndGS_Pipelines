...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    $temp =~ s/\.bam$//  ||  die; 
    say   "\t......$BAMfiles_g[$i]";
    system("  java  -jar   $Picard_g   MarkDuplicates    REMOVE_DUPLICATES=true  VALIDATION_STRINGENCY=LENIENT INPUT=$input_g/$temp.bam    OUTPUT=$output_g/$temp.bam    METRICS_FILE=$output_g/$temp.marked_dup_metrics.txt   >> $output_g/$temp.REMOVE_DUPLICATES.runLog     2>&1 ");
}
###################################################################################################################################################################################################
## VALIDATION_STRINGENCY=LENIENT
#Picard_g=/media/yp/yongpeng-2TB/MyProgramFiles/3_BAM_QC/picard/picard.jar
#input_g=4_finalBAM/1_BWA
#output_g=5_rmDups/1_BWA
#temp=Batch6_Ctrl.CBF04_S4
#temp=Batch6_Ctrl.CBF54_S6
#temp=Batch6_Ctrl.CBF56_S8
#temp=Batch6_Ctrl.CBF76_S4
