...
my $output_g   = '';  ## such as "4_finalBAM/1_BWA/Another_QC"
...
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 4: Assess the quality of BAM files to identify possible mapping errors or biases by using 12 softwares:
        ...
        Usage:
               perl  BAM_QC.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]  
        For instance:
               nohup time  perl BAM_QC.pl   -genome hg38   -in 4_finalBAM/1_BWA   -out 4_finalBAM/1_BWA/Another_QC     > BAM_QC.runLog.1_BWA.txt  2>&1    &  
...
$output_g = '4_finalBAM/1_BWA/Another_QC';         ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  BAM_QC.pl  -help' \n";
    exit 0;
}
...
sub  myQC_BAM_1  {
    ...
    system( "multiqc    --title Aligner    --verbose  --export   --outdir $MultiQC5   --pdf    --ignore $foldername   $dir1   >> $MultiQC5/MultiQC.Aligner.runLog    2>&1" );
}
###################################################################################################################################################################################################
