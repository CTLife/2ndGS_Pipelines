...
my $HELP = '
        ...
        Usage:
               perl WG-ssDNA_2.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               nohup time  perl WG-ssDNA_2.pl   -in 1_rawFASTQ   -out 2_finalFASTQ    > WG-ssDNA_2.runLog.txt  2>&1    &
...
## Version Infromation
my $version = "    The 2nd Step, version 1.0,  2022-08-01.";
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl WG-ssDNA_2.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        system("trim_galore  --quality 20   --phred33  --length 20    --clip_R1 12   --clip_R2 12           --output_dir  $output_g    --paired  $output_g/Trimmomatic/$end1  $output_g/Trimmomatic/$end2          >> $output2_g/$temp.trim_galore.runLog  2>&1"); 
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {   
        ...
        system("trim_galore  --quality 20   --phred33  --length 20      --clip_R1 12        --three_prime_clip_R1 0     --three_prime_clip_R2 0     --output_dir  $output_g       $output_g/Trimmomatic/$singleEnd_g[$i]      >> $output2_g/$temp.trim_galore.runLog  2>&1");   
