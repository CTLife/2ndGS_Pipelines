...
my $input_g    = '';  ## such as "3_rawBAM/1_BWA"
my $output_g   = '';  ## such as "4_finalBAM/1_BWA"
...
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 4: Only the mapped reads with MAPQ>1 are retained. And reads on other chromosomes are removed.
        ...
        Usage:
               perl  WG-ssDNA_4.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]  
        For instance:
               nohup time  perl WG-ssDNA_4.pl   -genome hg38   -in 3_rawBAM/1_BWA   -out 4_finalBAM/1_BWA     > WG-ssDNA_4.runLog.1_BWA.txt  2>&1    &  
...
## Version Infromation
my $version = "    The 4th Step, version 1.0,  2022-08-01.";
...
$input_g  = '3_rawBAM/1_BWA';          ## This is only an initialization value or suggesting value, not default value.
$output_g = '4_finalBAM/1_BWA';        ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  WG-ssDNA_4.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    $temp =~ s/\.bam$//  ||  die; 
    say   "\t......$BAMfiles_g[$i]";
    system("samtools  view   -h    --threads $numCores_g    -q 1   -L $genome_g.genome.chromsize.selected.bed   -o $output_g/$temp.bam   $input_g/$temp.bam    >> $output2_g/$temp.runLog     2>&1  ");  
}
###################################################################################################################################################################################################
