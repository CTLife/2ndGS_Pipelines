...
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 4: Mapping reads to the reference genome by using BWA or Bowtie2.
        ...
        Usage:
               perl WG-ssDNA_3.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]  
        For instance:
               nohup time  perl WG-ssDNA_3.pl   -genome hg38   -in 2_finalFASTQ   -out 3_rawBAM     > WG-ssDNA_3.runLog.txt  2>&1    &  
...
## Version Infromation
my $version = "    The 3rd Step, version 1.0,  2022-08-01.";
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  WG-ssDNA_3.pl  -help' \n";
    exit 0;
}
...
my  $Lambda_index_g   = "$commonPath_g/bowtie2/RefGenomes/lambda_phage/lambda_phage";
my  $Lambda_A2G_g     = "$commonPath_g/hisat-3n/RefGenomes/3N_A2G/HFM_index/lambda_phage/lambda_phage";
my  $BWA_index_g      = "";
my  $Bowtie2_index_g  = "";
my  $HISAT2_index_g   = "";
my  $HISAT3N_index_g  = "";
...
if($genome_g eq "hg38") {
    ...
    $HISAT2_index_g    = "$commonPath_g/hisat-3n/RefGenomes/4N/HFM_index/hg38/hg38";
    $HISAT3N_index_g   = "$commonPath_g/hisat-3n/RefGenomes/3N_A2G/HFM_index/hg38/hg38";
    $STAR_index_g      = "$commonPath_g/STAR/RefGenomes/hg38";
}

if($genome_g eq "mm39") {
    $BWA_index_g       = "$commonPath_g/bwa/RefGenomes/mm39/mm39";  
    $Bowtie2_index_g   = "$commonPath_g/bowtie2/RefGenomes/mm39/mm39";
    $HISAT2_index_g    = "$commonPath_g/hisat-3n/RefGenomes/4N/HFM_index/mm39/mm39";
    $HISAT3N_index_g   = "$commonPath_g/hisat-3n/RefGenomes/3N_A2G/HFM_index/mm39/mm39";
    $STAR_index_g      = "$commonPath_g/STAR/RefGenomes/mm39";
}
...
###################################################################################################################################################################################################
my $lambda_2_g     = "$output_g/0A_ToLambda";
my $lambda_SE_2_g  = "$output_g/0B_ToLambda_A2G";
&myMakeDir($lambda_2_g);
&myMakeDir($lambda_SE_2_g);
{ ## Start bismark
say   "\n\n\n\n\n\n##################################################################################################";
say   "Mapping reads to the reference genome by using Bowtie2 and hisat-3n (Mapping To the Lambda Genome) ......";
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        say    "\t......$pairedEnd_g[$i]";
        say    "\t......$pairedEnd_g[$i+1]";
        ...
        system("bowtie2     --threads $numCores_g   -q   --phred33      --local   --mp 4                                                        -x $Lambda_index_g    -1 $input_g/$end1    -2 $input_g/$end2   -S $lambda_2_g/$temp.sam       >> $lambda_2_g/$temp.runLog  2>&1");
        system("hisat-3n    --threads $numCores_g   --base-change A,G   --no-repeat-index   --summary-file  $lambda_SE_2_g/$temp.summary.txt    -x $Lambda_A2G_g      -1 $input_g/$end1    -2 $input_g/$end2   -S $lambda_SE_2_g/$temp.sam    >> $lambda_SE_2_g/$temp.runLog  2>&1");
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,   $end2\n";
        system("bwa mem  -t $numCores_g    -T 1   -L 3,3   -B 3       $BWA_index_g   $input_g/$end1  $input_g/$end2    > $BWA_g/$temp.sam");
}
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("bwa mem  -t $numCores_g      -T 1   -L 3,3   -B 3      $BWA_index_g   $input_g/$singleEnd_g[$i]   > $BWA_g/$temp.sam"); 
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,   $end2\n";
        system("bowtie2     --threads $numCores_g   -q   --phred33   --un-conc $Bowtie2_g/$temp      --no-mixed      --no-discordant              -x $Bowtie2_index_g    -1 $input_g/$end1        -2 $input_g/$end2       -S $Bowtie2_g/$temp.sam           >> $Bowtie2_g/$temp.runLog         2>&1");
        system("hisat-3n    --threads $numCores_g  --base-change A,G   --no-repeat-index   --summary-file  $Bowtie2_g/$temp.hisat2.summary.txt    -x $HISAT3N_index_g    -1 $Bowtie2_g/$temp.1    -2 $Bowtie2_g/$temp.2   -S $Bowtie2_g/$temp.hisat2.sam    >> $Bowtie2_g/$temp.hisat2.runLog  2>&1");
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,   $end2\n";
        system("STAR  --runMode alignReads    --runThreadN $numCores_g   --readFilesCommand zcat  --alignSJDBoverhangMin 1   --outSAMmultNmax 1   --outMultimapperOrder Random     --outFilterMismatchNoverReadLmax 0.25  --outFilterMismatchNmax 999  --outFilterMultimapNmax 100     --alignIntronMax 1     --outFileNamePrefix  $STAR_3_g/$temp.   --genomeDir $STAR_index_g  --readFilesIn $input_g/$end1  $input_g/$end2   >> $STAR_3_g/$temp.runLog  2>&1 ");
}
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("STAR  --runMode alignReads    --runThreadN $numCores_g   --readFilesCommand zcat  --alignSJDBoverhangMin 1   --outSAMmultNmax 1   --outMultimapperOrder Random     --outFilterMismatchNoverReadLmax 0.25  --outFilterMismatchNmax 999  --outFilterMultimapNmax 100     --alignIntronMax 1     --outFileNamePrefix  $STAR_3_g/$temp.   --genomeDir $STAR_index_g  --readFilesIn $singleEnd_g[$i]   >> $STAR_3_g/$temp.runLog  2>&1 ");
}
&myQC_BAM_1($STAR_3_g);

&myMakeDir("$output2_g/3_MultiQC_STAR");
system( "multiqc    --title STAR        --verbose  --export   --outdir $output2_g/3_MultiQC_STAR     --ignore QC_Results   --pdf    $STAR_3_g     >> $output2_g/3_MultiQC_STAR/MultiQC.STAR.runLog    2>&1" );
