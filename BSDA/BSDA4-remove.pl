...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "5-finalBAM/2A_Bismark"
my $output_g = '';  ## such as "5-finalBAM-remove/2A_Bismark"
...
my $HELP = '
        ...
        Step 4: Only the mapped reads with MAPQ>10 are retained.
        ...
        Usage:
               perl  BSDA4-remove.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               perl  BSDA4-remove.pl   -in 5-finalBAM/2A_Bismark   -out 5-finalBAM-remove/2A_Bismark    > BSDA4-remove.runLog
...
## Version Infromation
my $version = "    The 4th Step of BSDA (BS-Seq Data Analyzer), version 0.9.4,  2018-02-01.";
...
$input_g  = '5-finalBAM/2A_Bismark';            ## This is only an initialization value or suggesting value, not default value.
$output_g = '5-finalBAM-remove/2A_Bismark';     ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  BSDA4-remove.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {
        next unless $inputFiles_g[$i] =~ m/\.sam$/;
        ...
        $temp =~ m/^removed_(\d+)_($pattern_g)_(Rep[1-9])/   or  die   "wrong-1: ## $temp ##";
        $temp =~ m/_(Rep[1-9])\.sam$/   or  die   "wrong-2: ## $temp ##";
        if($temp !~ m/^(removed_(\d+)_($pattern_g)_(Rep[1-9]))(_[1-2])?\.bam$/) {
             $fileNameBool = 0;
        }
}
...
for ( my $i=0; $i<=$#groupFiles; $i++ ) {
    $groupFiles[$i] =~ m/^removed_(\d+)_($pattern_g)_(Rep[1-9])/  or  die;
    my $n1 = $1;
    $n1>=1  or  die;
    if($noteGroup != $n1) {say "\n\t\tGroup $n1:";  $numGroup++; }
    say  "\t\t\t$groupFiles[$i]";
...
###################################################################################################################################################################################################
say   "\n\n\n\n\n\n##################################################################################################";
say   "Detecting SAM files in input folder ......";
my @BAMfiles_g = ();
{
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {     
    ...
    next unless $inputFiles_g[$i] !~ m/unpaired/;
    say    "\t......$inputFiles_g[$i]"; 
    $inputFiles_g[$i] =~ m/^removed_(\d+)_($pattern_g)_(Rep[1-9])\.sam$/  or  die;  
    $BAMfiles_g[$#BAMfiles_g+1] =  $inputFiles_g[$i];
    say        "\t\t\t\tSAM file:  $inputFiles_g[$i]\n";
    say   seqFiles_FH  "SAM file:  $inputFiles_g[$i]\n";
}

say   seqFiles_FH  "\n\n\n\n\n";  
say   seqFiles_FH  "All SAM files:@BAMfiles_g\n\n\n";
say        "\t\t\t\tAll SAM files:@BAMfiles_g\n\n";
my $num1 = $#BAMfiles_g + 1;
say seqFiles_FH   "\nThere are $num1 SAM files.\n";
say         "\t\t\t\tThere are $num1 SAM files.\n";
}
###################################################################################################################################################################################################
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    $temp =~ s/\.sam$//  ||  die; 
    say   "\t......$BAMfiles_g[$i]";
    system(`samtools  view   -h    --threads $numCores_g    -q 10    -o $output_g/$temp.sam   $input_g/$temp.sam    >> $output2_g/$temp.runLog     2>&1  `);  
}
###################################################################################################################################################################################################
