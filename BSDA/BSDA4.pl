...
my $input_g  = '';  ## such as "4-rawBAM/1_Bismark"
my $output_g = '';  ## such as "5-MAPQ3/1_Bismark"
...
my $HELP = '
        ...
        Step 4: Only the mapped reads with MAPQ>3 are retained and reads on chr_random, chr_alt, chrUn and chrM, all of them are removed.
        ...
        Usage:
               perl  BSDA4.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]
        For instance:
               perl  BSDA4.pl   -genome hg38   -in 4-rawBAM/1_Bismark   -out 5-MAPQ3/1_Bismark    > BSDA4.runLog
...
## Version Infromation
my $version = "    The 4th Step of BSDA (BS-Seq Data Analyzer), version 0.9.0, 2017-10-01.";
...
$input_g  = '4-rawBAM/1_Bismark';    ## This is only an initialization value or suggesting value, not default value.
$output_g = '5-MAPQ3/1_Bismark';     ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  BSDA4.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    $temp =~ s/\.bam$//  ||  die; 
    say   "\t......$BAMfiles_g[$i]";
    system(`samtools  view   -h    --threads $numCores_g    -q 3    -o $output_g/$temp.t.sam   $input_g/$temp.bam    >> $output2_g/$temp.runLog     2>&1  `);  
