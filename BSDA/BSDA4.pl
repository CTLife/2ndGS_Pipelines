...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "4-rawBAM/2A_Bismark"
my $output_g = '';  ## such as "5-finalBAM/2A_Bismark"
...
my $HELP = '
        ...
        Step 4: Only the mapped reads with MAPQ>10 are retained and reads on chr_random, chr_alt, chrUn and chrM, all of them are removed.
        ...
        Usage:
               perl  BSDA4.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               perl  BSDA4.pl   -in 4-rawBAM/2A_Bismark   -out 5-finalBAM/2A_Bismark    > BSDA4.runLog
...
$input_g  = '4-rawBAM/2A_Bismark';       ## This is only an initialization value or suggesting value, not default value.
$output_g = '5-finalBAM/2A_Bismark';     ## This is only an initialization value or suggesting value, not default value.
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    $temp =~ s/\.bam$//  ||  die; 
    say   "\t......$BAMfiles_g[$i]";
    system(`samtools  view   -h    --threads $numCores_g    -q 10    -o $output_g/$temp.t.sam   $input_g/$temp.bam    >> $output2_g/$temp.runLog     2>&1  `);  
