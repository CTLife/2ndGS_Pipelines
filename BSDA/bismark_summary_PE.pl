...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "2A_Bismark", global variable.   
my $output_g = '';  ## such as "2A_Bismark_Summary", global variable. 
...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------                     
        Usage:
               perl  bismark_summary_PE.pl    [-version]    [-help]     [-in inputDir]    [-out outDir]
        For instance:
               perl  bismark_summary_PE.pl    -in 2A_Bismark   -out 2A_Bismark_Summary    > bismark_summary_PE.runLog  
...
## Version Infromation
my $version = "     bismark_summary_PE, version 0.9.4,  2018-02-01.";
...
$input_g  = '2A_Bismark';     ## This is only an initialization value or suggesting value, not default value.
$output_g = '2A_Bismark_Summary';     ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  bismark_summary_PE.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) { 
        next unless $inputFiles_g[$i] =~ m/^(\d+)_($pattern_g)_Rep/;  
        ...
        for ( my $j=0; $j<=$#lines1; $j++ ) { 
            if($lines1[$j] =~ m/^Sequence\s+pairs\s+analysed\s+in\s+total:\s+(\d+)\n$/i) {$TotalSeqPairs_2 = $1;}
            if($lines1[$j] =~ m/^Number\s+of\s+paired-end\s+alignments\s+with\s+a\s+unique\s+best\s+hit:\s+(\d+)\n$/i) {$UniqueBestHit_3 = $1;}
            if($lines1[$j] =~ m/^Sequence\s+pairs\s+with\s+no\s+alignments\s+under\s+any\s+condition:\s+(\d+)\n$/i) {$NoAlignments_5 = $1;}
            if($lines1[$j] =~ m/^Sequence\s+pairs\s+did\s+not\s+map\s+uniquely:\s+(\d+)\n$/i) {$MultiMapped_7 = $1;}
            if($lines1[$j] =~ m/^Sequence\s+pairs\s+which\s+were\s+discarded\s+because\s+genomic\s+sequence\s+could\s+not\s+be\s+extracted:\s+(\d+)\n$/i) {$notExtracted_8a = $1;}
