...
my $HELP_g = '
        ...
        Step 5: Potential PCR duplicates are marked using "MarkDuplicates" from "Picards".
                Finally, only the UMR4 with MAPQ is more than 30 are kept.
        ...
        Usage:  
               perl  RASDA5.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]  
        For instance: 
               perl  RASDA5.pl    -in 5-MAPQ30/5_STAR     -out 6-FinalBAM/5_STAR      >> RASDA5.runLog  2>&1
...
## Version Information
my $version_g = "  The Fifth Step of RASDA (RNA-Seq Data Analyzer), version 0.7.3, 2016-06-01.";
...
my $input_g  = '5-MAPQ30/5_STAR';         ## This is only an initialization  value or suggesting value, not default value.
my $output_g = '6-FinalBAM/5_STAR';       ## This is only an initialization  value or suggesting value, not default value.
...
sub  myQC_BAM_1  {
    ...
    for ( my $i=0; $i<=$#Files; $i++ ) {
        next unless $Files[$i] =~ m/\.t\.bam$/;
        next unless $Files[$i] !~ m/^[.]/;
        next unless $Files[$i] !~ m/[~]$/;
        my $temp = $Files[$i];
        say    "\t......$temp";
        $temp =~ s/\.t\.bam$//  ||  die;
        system("samtools  sort  -m 2G  -o $dir1/$temp.bam   --output-fmt bam  -T $dir1/yp_$temp   --threads $numCores_g    $dir1/$temp.t.bam    >>$SAMtools/$temp.runLog    2>&1");
