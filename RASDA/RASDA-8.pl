...
my $HELP_g = '
        ...
        Usage:  
               perl  RASDA-8.pl    [-v]    [-h]    [-i inputDir]    [-o outDir]   
        For instance: 
                     perl  RASDA-8.pl    -i 7-FinalBAM/2-Subjunc          -o 9-BED                      
                     perl  RASDA-8.pl    --input 7-FinalBAM/2-Subjunc     --output 9-BED     
                     perl  RASDA-8.pl    --input 7-FinalBAM/2-Subjunc     --output 9-BED    >> RASDA-8.runLog  2>&1
...
########## Version Infromation ##########
my $version_g = "  The Eighth Step of RASDA (RNA-Seq Data Analyzer), version 0.62, 2016-01-13.";
...
if($boole_g == 1) {
    ...
    print   '    Please see help message by using "perl  RASDA-8.pl  -h". ';
    print "\n\n";
    exit 0;
}
...
for(my $i=0; $i<=$#inputFiles; $i++) {
    ...
    system("qualimap   rnaseq  --algorithm uniquely-mapped-reads    -bam $input_g/$temp.bam    -gtf 0-Other/mm9-RefSeq.gtf   -oc computedCounts.txt  -outdir $output2_g/$temp-2  --sequencing-protocol non-strand-specific    --java-mem-size=5G  --paired"); 
}
