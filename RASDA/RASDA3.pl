...
my $HELP = '
        ...
        Step 3: Mapping reads to the reference genome by using 10 softwares (mappers or aligners): 
                ...
                And aggregate the results from SAMtools, FastQC, Qualimap,  Preseq, Picard,  goleft, Bamtools and STAR 
        ...
        Usage:
               perl  RASDA3.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]
        For instance:
               perl  RASDA3.pl   -genome hg38   -in 3-finalFASTQ   -out 4-rawBAM    > RASDA3.runLog
...
## Version Infromation
my $version = "    The 3rd Step of RASDA (RNA-Seq Data Analyzer), version 0.9.4,  2018-02-01.";
...
###################################################################################################################################################################################################
## Context specific:
my  $commonPath_g      = "/media/yp/biox1/.MyProgramFiles/4_ChIPseq/5-Mapping";
...
my  $BBMap_index_g     = "/media/yp/biox1/.MyProgramFiles/4_ChIPseq/3-Remove-Correct/bbmap/RefGenomes/$genome_g";
...
my  $commonPath2_g     = "/media/yp/biox1/.MyProgramFiles/5-RNAseq";
...
my  $STAR_ensembl_index_g      = "$commonPath2_g/STAR/RefGenomes/$genome_g.ensembl";
my  $HISAT2_ensembl_index_g    = "$commonPath2_g/hisat2/RefGenomes/$genome_g.ensembl/$genome_g.ensembl";
...
&myMakeDir("$Kallisto_1B_g/QC_Results/MultiQC");
system( "multiqc    --title Aligner    --verbose  --export   --outdir $Kallisto_1B_g/QC_Results/MultiQC    --ignore QC_Results   $Kallisto_1B_g   >> $Kallisto_1B_g/QC_Results/MultiQC/MultiQC.Aligner.runLog    2>&1" );     
...
&myMakeDir("$Salmon_2B_g/QC_Results/MultiQC");
system( "multiqc    --title Aligner    --verbose  --export   --outdir $Salmon_2B_g/QC_Results/MultiQC    --ignore QC_Results   $Salmon_2B_g   >> $Salmon_2B_g/QC_Results/MultiQC/MultiQC.Aligner.runLog    2>&1" );     
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,   $end2\n";
        system("STAR  --runMode alignReads    --runThreadN $numCores_g    --sjdbGTFfile 0-Other/Shortcuts/$genome_g/$genome_g.ensembl.cDNA.GTF    --quantMode   GeneCounts       --clip3pNbases $trim3_g    --clip5pNbases $trim5_g    --outFileNamePrefix  $STAR_3B_g/$temp.   --genomeDir $STAR_ensembl_index_g  --readFilesIn $inputDir2/$end1.fastq  $inputDir2/$end2.fastq   >>$STAR_3B_g/$temp.runLog  2>&1 ");
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $singleEnd_g[$i] =~ m/^((\d+)_($pattern_g)_(Rep[1-9]))\.fastq$/   or  die;
        my $temp = $1;
        system("STAR  --runMode alignReads   --runThreadN $numCores_g    --sjdbGTFfile 0-Other/Shortcuts/$genome_g/$genome_g.ensembl.cDNA.GTF     --quantMode   GeneCounts        --clip3pNbases $trim3_g    --clip5pNbases $trim5_g    --outFileNamePrefix  $STAR_3B_g/$temp.   --genomeDir $STAR_ensembl_index_g  --readFilesIn $inputDir2/$temp.fastq   >>$STAR_3B_g/$temp.runLog  2>&1 ");
        system("rename  s/Aligned.out.sam/sam/   $STAR_3B_g/*Aligned.out.sam");
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,  $end2\n";
        system("hisat2         --threads $numCores_g   -q   --phred33   --end-to-end    -x $HISAT2_ensembl_index_g   --trim5 $trim5_g  --trim3 $trim3_g    -1 $inputDir2/$end1.fastq        -2 $inputDir2/$end2.fastq     -S $HISAT2_4B_g/$temp.sam    >>$HISAT2_4B_g/$temp.runLog  2>&1");
}
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $singleEnd_g[$i] =~ m/^((\d+)_($pattern_g)_(Rep[1-9]))\.fastq$/   or  die;
        my $temp = $1;
        system("hisat2        --threads $numCores_g   -q   --phred33   --end-to-end    -x $HISAT2_ensembl_index_g   --trim5 $trim5_g  --trim3 $trim3_g    -U $inputDir2/$temp.fastq                                    -S $HISAT2_4B_g/$temp.sam    >>$HISAT2_4B_g/$temp.runLog  2>&1");
}
...
&myMakeDir("$RSEM_5A_g/QC_Results/MultiQC");
system( "multiqc    --title Aligner    --verbose  --export   --outdir $RSEM_5A_g/QC_Results/MultiQC    --ignore QC_Results   $RSEM_5A_g   >> $RSEM_5A_g/QC_Results/MultiQC/MultiQC.Aligner.runLog    2>&1" );     
...
&myMakeDir("$RSEM_5B_g/QC_Results/MultiQC");
system( "multiqc    --title Aligner    --verbose  --export   --outdir $RSEM_5B_g/QC_Results/MultiQC    --ignore QC_Results   $RSEM_5B_g   >> $RSEM_5B_g/QC_Results/MultiQC/MultiQC.Aligner.runLog    2>&1" );     
