...
my $HELP_g = '
        ...
        Usage:  
               perl  RASDA-7.pl    [-v]    [-h]    [-i inputDir]    [-o outDir]   [-L n]
        For instance: 
                     perl  RASDA-7.pl    -i 7-FinalBAM/2-Subjunc          -o 8-BigWig          -L 50            
                     perl  RASDA-7.pl    --input 7-FinalBAM/2-Subjunc     --output 8-BigWig    --fragLen 50 
                     perl  RASDA-7.pl    --input 7-FinalBAM/2-Subjunc     --output 8-BigWig    --fragLen 50  >> RASDA-7.runLog  2>&1
...
########## Version Infromation ##########
my $version_g = "  The Seventh Step of RASDA (RNA-Seq Data Analyzer), version 0.62, 2016-01-13.";
...
my $fragLen_g = 50;                          ## This is only an initialization  value or suggesting value, not default value.
...
if($boole_g == 1) {
    ...
    print   '    Please see help message by using "perl  RASDA-7.pl  -h". ';
    print "\n\n";
    exit 0;
}
...
my $BigWig1  = "$output_g/1-NoExtend-E7";
my $BigWig2  = "$output_g/2-NoExtend-1X";
my $BigWig3  = "$output_g/3-NoExtend-RPKM";
...
for(my $i=0; $i<=$#inputFiles; $i++) {
    ...
    system(" bamCoverage   --numberOfProcessors 4    --bam $input_g/$temp.bam     --outFileName $BigWig1/$temp.bw     --outFileFormat bigwig       --normalizeTo1x  $norFact                --fragmentLength $fragLen_g      --binSize 1   ");                
    system(" bamCoverage   --numberOfProcessors 4    --bam $input_g/$temp.bam     --outFileName $BigWig2/$temp.bw     --outFileFormat bigwig       --normalizeTo1x  $EffectiveGenomeSize    --fragmentLength $fragLen_g      --binSize 20  ");                    
    system(" bamCoverage   --numberOfProcessors 4    --bam $input_g/$temp.bam     --outFileName $BigWig3/$temp.bw     --outFileFormat bigwig       --normalizeUsingRPKM                     --fragmentLength $fragLen_g      --binSize 20  ");                                             
}
...
system(" bamCorrelate  bins  --bamfiles @BAMFiles   -o $spearman/heatmap_spearman.svg   --corMethod spearman    --labels @BAMFiles2    --binSize 10000   --plotTitle Spearman_rank_correlation_coefficient  --numberOfProcessors 4   --outFileCorMatrix $spearman/spearman_matrix.txt       --plotNumbers   --fragmentLength $fragLen_g ");                                                
system(" bamCorrelate  bins  --bamfiles @BAMFiles   -o $pearson/heatmap_pearson.svg     --corMethod pearson     --labels @BAMFiles2    --binSize 10000   --plotTitle Pearson_rank_correlation_coefficient   --numberOfProcessors 4   --outFileCorMatrix $pearson/pearson_matrix.txt         --plotNumbers   --fragmentLength $fragLen_g ");    
system(" bamFingerprint      --bamfiles @BAMFiles   --plotFile  $bamFinger/Finger.svg    --binSize 500   --labels @BAMFiles2    --fragmentLength $fragLen_g   --numberOfSamples 1000000   --plotTitle bamFingerprint_Figure  --numberOfProcessors 4 ");
