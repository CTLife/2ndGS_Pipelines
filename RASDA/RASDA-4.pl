...
for(my $i=0; $i<=$#inputFiles; $i++) {
    ...
    system("qualimap  bamqc  -bam $output_HISAT2/All_$temp.bam   -c    -gd MOUSE  -outdir $qualimapDir/All_$temp      >>$output2_HISAT2/$temp.runLog    2>&1");
    ...
    system("qualimap  bamqc  -bam $output_HISAT2/$temp.bam   -c    -gd MOUSE  -outdir $qualimapDir/$temp      >>$output2_HISAT2/$temp.runLog    2>&1");
    ...
    system("qualimap  bamqc  -bam $output_HISAT2/Removed_$temp.bam   -c    -gd MOUSE  -outdir $qualimapDir/Removed_$temp      >>$output2_HISAT2/$temp.runLog    2>&1");
...
for(my $i=0; $i<=$#inputFiles; $i++) {
    ...
    if ( !( -e "$qualimapDir/All_$temp")     )   { mkdir  "$qualimapDir/All_$temp"   ||  die; }       
    system("qualimap  bamqc  -bam $output_Subjunc/All_$temp.bam   -c    -gd MOUSE  -outdir $qualimapDir/All_$temp      >>$output2_Subjunc/$temp.runLog    2>&1");
    system("samstat   $output_Subjunc/All_$temp.bam       >> $output2_Subjunc/$temp.runLog    2>&1");
    system("echo    '\n\n\n\n\n\n'                        >> $output2_Subjunc/$temp.runLog    2>&1");

    if ( !( -e "$qualimapDir/$temp")     )   { mkdir  "$qualimapDir/$temp"   ||  die; }       
    system("qualimap  bamqc  -bam $output_Subjunc/$temp.bam   -c    -gd MOUSE  -outdir $qualimapDir/$temp      >>$output2_Subjunc/$temp.runLog    2>&1");
    system("samstat   $output_Subjunc/$temp.bam       >> $output2_Subjunc/$temp.runLog    2>&1");
    system("echo    '\n\n\n\n\n\n'                        >> $output2_Subjunc/$temp.runLog    2>&1");   

    if ( !( -e "$qualimapDir/Removed_$temp")     )   { mkdir  "$qualimapDir/Removed_$temp"   ||  die; }       
    system("qualimap  bamqc  -bam $output_Subjunc/Removed_$temp.bam   -c    -gd MOUSE  -outdir $qualimapDir/Removed_$temp      >>$output2_Subjunc/$temp.runLog    2>&1");
...
for(my $i=0; $i<=$#inputFiles; $i++) {
    ...
    if ( !( -e "$qualimapDir/All_$temp")     )   { mkdir  "$qualimapDir/All_$temp"   ||  die; }       
    system("qualimap  bamqc  -bam $output_STAR/All_$temp.bam   -c    -gd MOUSE  -outdir $qualimapDir/All_$temp      >>$output2_STAR/$temp.runLog    2>&1");
    system("samstat   $output_STAR/All_$temp.bam       >> $output2_STAR/$temp.runLog    2>&1");
    system("echo    '\n\n\n\n\n\n'                        >> $output2_STAR/$temp.runLog    2>&1");

    if ( !( -e "$qualimapDir/$temp")     )   { mkdir  "$qualimapDir/$temp"   ||  die; }       
    system("qualimap  bamqc  -bam $output_STAR/$temp.bam   -c    -gd MOUSE  -outdir $qualimapDir/$temp      >>$output2_STAR/$temp.runLog    2>&1");
    system("samstat   $output_STAR/$temp.bam       >> $output2_STAR/$temp.runLog    2>&1");
    system("echo    '\n\n\n\n\n\n'                        >> $output2_STAR/$temp.runLog    2>&1");   

    if ( !( -e "$qualimapDir/Removed_$temp")     )   { mkdir  "$qualimapDir/Removed_$temp"   ||  die; }       
    system("qualimap  bamqc  -bam $output_STAR/Removed_$temp.bam   -c    -gd MOUSE  -outdir $qualimapDir/Removed_$temp      >>$output2_STAR/$temp.runLog    2>&1");
